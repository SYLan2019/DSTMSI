...
from utils import load_metr_la_rdata, calculate_random_walk_matrix, load_pems04_data, load_pems08_data, \
    load_pemsbay_data, load_chengdu_data, load_sedata, load_bjair_data
import torch
...
def parse_arg():
    ...
    parser.add_argument('--dataset',type=str,default='bjair',help='name of the datasets,select from metr,nrel,ushcn,sedata of pemsbay')
    parser.add_argument('--n_s',type=int,default=18,help='sampled space dimension')  # n_s=n_o+n_m
    ...
    parser.add_argument('--n_m',type=int,default=9,help='number of mask node during training')
    parser.add_argument('--n_u',type=int,default=17,help='target locations,n_u locations will be deleted from training data')
    parser.add_argument('--epochs',type=int,default=300,help='max training episode')
    ...
    parser.add_argument('--dropout',type=float,default=0.1)   #可以改
...
class gwnet(nn.Module): #输入bnt,->b1nt->b32nt->b32nt'->b256nt''-->b512nt'''->b24n1-->b1n24->bnt,输出bnt
    ...
    def forward(self, input,A_q,A_h):   # btn3
        ...
        input=torch.einsum("wn,bfnt->bfwt",A_q,input.clone())
...
def load_data(dataset):
    ...
    if dataset == 'metr':
        ...
        A, X = load_sedata()
        X=X.reshape((323,8064,1))
        X=X.transpose(1,0,2)
        A = A.astype('float32')
        X = X.astype('float32')
    elif dataset == 'chengdu':
        A, X = load_chengdu_data()
        X = X[:, 0:1, :]    #n 1 t
        X=X.transpose(2,0,1)    #tnf
    elif dataset=='bjair':
        X,A,missing_index=load_bjair_data()
        X[missing_index] = 0
